...
@router.put("/{task_id}")
async def task_update(task_id: int, task: TaskCreateSchema, db: Session = Depends(get_db), user_id: int = Depends(get_authenticated_user)):
    current_task = db.query(Task).filter_by(id = task_id, user_id = user_id).first()
    if not current_task:
        raise HTTPException(detail="task not found", status_code = status.HTTP_400_BAD_REQUEST)
    current_task.title = task.title
    db.commit()
    db.refresh(current_task)
    return JSONResponse(content={"message": "successfully edited"}, status_code = status.HTTP_200_OK)


@router.delete("/del/{task_id}")
async def task_delete(task_id: int, user_id: int = Depends(get_authenticated_user), db: Session = Depends(get_db)):
