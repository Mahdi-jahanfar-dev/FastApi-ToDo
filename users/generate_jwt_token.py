...
def get_authenticated_user(credential: HTTPAuthorizationCredentials = Depends(security)) -> int:
    ...
    try:
        ...
        if decoded_token["type"] != "access":
            raise HTTPException(detail="token type is not access", status_code=status.HTTP_401_UNAUTHORIZED)
...
def get_access_token(credential: HTTPAuthorizationCredentials = Depends(security)) -> str:
    
    token = credential.credentials
    
    try:
        decoded_refresh_token = jwt.decode(token, settings.JWT_SECRET_KEY, algorithms=["HS256"])
        
        if decoded_refresh_token["type"] != "refresh":
            raise HTTPException(detail="token type is not refresh", status_code=status.HTTP_401_UNAUTHORIZED)
